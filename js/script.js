...
deseaSeguir = prompt("¿Desea seguir comprando?"+"\n"+"Si"+"\n"+"No")
if (deseaSeguir === "Si" || deseaSeguir === "si" || deseaSeguir === "SI" || deseaSeguir === "sI"){
    SALIR = false
}

else if (deseaSeguir === "No" || deseaSeguir !== "no" || deseaSeguir !== "NO" || deseaSeguir === "nO"){
    SALIR = true
}

else if (deseaSeguir !== "Si" || deseaSeguir !== "No" || deseaSeguir !== "no" || deseaSeguir !== "NO" || deseaSeguir === "si" || deseaSeguir === "SI" || deseaSeguir === "sI" || deseaSeguir === "nO"){
    deseaSeguir = prompt("¿Desea seguir comprando?"+"\n"+"Si"+"\n"+"No");
}
}
...
function pedirStock(productoSelec){
    let cantidad = Number(prompt("Cuantas "+productoSelec.nombre+" deseas llevar"+"\n"+"Stock: "+productoSelec.stock))
    let descuento1 = "DESCUENTO"
    let total = Number(cantidad*productoSelec.valor)
    let pedirDescuento = prompt("El total de tu compra es de "+total+"\n"+"Si tienes algun cupon de descuento ingresalo a continuación")
    if (pedirDescuento == descuento1){
        total = (cantidad*productoSelec.valor)-10000
        alert ("Felicidades! tu descuento de 10000 CLP a sido activado")
        if (cantidad > productoSelec.stock){
            cantidad = prompt("Ingrese un stock valido")
            alert("Haz agregado "+cantidad+" "+productoSelec.nombre+"\n"+"Precio Total: "+total+" (Descuento de 10000 CLP ya incluido)")
        }
        else{
            alert("Haz agregado "+cantidad+" "+productoSelec.nombre+"\n"+"Precio Total: "+total+" (Descuento de 10000 CLP ya incluido)")
        }
